...
class NewItemForm(FlaskForm):
    ...
    description = TextAreaField("Description",validators=[InputRequired("Input is required"),DataRequired("Data is required"),Length(min=5,max=20,message="Input must be between 5 and 40 characters long")])
...
def new_item():
    ...
    if form.validate_on_submit():
        ...
        flash("Item {} has been succesfully submitted.".format(request.form.get("title")), "success")
...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect('db/dbmain.db')
    return db
